	...
	public function pruneDeadWorkers()
	{
		...
		foreach($workers as $worker) {
			if (is_object($worker)) {
                ...
                // Restore job to the worker, so that unregisterWorker() will mark it as failed
                $recovered_job_data = $worker->job();
                $worker->currentJob = new Resque_Job($recovered_job_data['queue'], $recovered_job_data['payload']);
                $worker->currentJob->worker = new Resque_Worker($queues);
                $worker->currentJob->worker->setId((string)$worker);
